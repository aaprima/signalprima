...
const MARKETS = {
  ...
  forex: {
    ...
    data: {
      EURUSD: { base: 1.0852, dp: 5, vol: 0.006, td: "EUR/USD", oanda: "EUR_USD" },
      GBPUSD: { base: 1.3319, dp: 5, vol: 0.007, td: "GBP/USD", oanda: "GBP_USD" },
      USDJPY: { base: 151.24, dp: 3, vol: 0.006, td: "USD/JPY", oanda: "USD_JPY" },
      AUDUSD: { base: 0.6581, dp: 5, vol: 0.007, td: "AUD/USD", oanda: "AUD_USD" },
      USDCHF: { base: 0.8847, dp: 5, vol: 0.006, td: "USD/CHF", oanda: "USD_CHF" },
      EURJPY: { base: 164.12, dp: 3, vol: 0.007, td: "EUR/JPY", oanda: "EUR_JPY" },
    },
  },
  emas: {
    label: "Emas",
    quick: ["XAUUSD", "XAGUSD"],
    data: {
      XAUUSD: { base: 4061, dp: 2, vol: 0.012, td: "XAU/USD", oanda: "XAU_USD" },
      XAGUSD: { base: 28.4, dp: 3, vol: 0.02, td: "XAG/USD", oanda: "XAG_USD" },
    },
  },
  saham_as: {
    label: "Saham AS",
...
// Ambil candle live lewat proxy Cloudflare Worker milik user (OANDA: Forex/Emas — feed broker real-time).
async function fetchOandaCandles(proxyUrl, oandaSymbol, tf, count) {
  const u = new URL(proxyUrl.replace(/\/+$/, "") + "/oanda_candles");
  u.searchParams.set("instrument", oandaSymbol);
  u.searchParams.set("granularity", OANDA_GRANULARITY[tf] || "H1");
  u.searchParams.set("count", String(count));
  const res = await fetch(u.toString());
  const data = await res.json();
  if (data.error || data.errorMessage) {
    throw new Error(data.errorMessage || data.error || "OANDA mengembalikan error");
  }
  if (!data.candles || !Array.isArray(data.candles)) {
    throw new Error("Format respons OANDA tidak dikenali");
  }
  const complete = data.candles.filter((c) => c.complete !== false);
  return complete.map((c, i) => ({
    i,
...
const GOLDAPI_SYMBOL = { XAUUSD: "XAU", XAGUSD: "XAG" };

// Harga real-time presisi khusus Emas/Perak dari GoldAPI.io (lewat proxy).
async function fetchGoldApiPrice(proxyUrl, symbol) {
  const u = new URL(proxyUrl.replace(/\/+$/, "") + "/goldapi_price");
  u.searchParams.set("symbol", symbol);
  const res = await fetch(u.toString());
  const data = await res.json();
  if (data.error) throw new Error(data.error);
  const price = data.price ?? data.mid_price ?? data.ask;
  if (typeof price !== "number") throw new Error("Format respons GoldAPI tidak dikenali");
  return price;
}

// Timpa candle terakhir dengan harga presisi (dari GoldAPI), supaya entry mendekati harga pasar riil
// sementara EMA/RSI/ATR tetap dihitung dari seluruh riwayat candle.
function applyPriceOverride(candles, price) {
...
// ---------------- Claude API helper (dipakai untuk baca gambar / Analisa Chart — selalu Anthropic) ----------------
...
async function askClaude({ system, userContent }, proxyUrl) {
  ...
  const data = await res.json();
  if (data.error) throw new Error(typeof data.error === "string" ? data.error : data.error.message || "Error dari AI");
  // Respons proxy sudah dinormalisasi jadi { text }. Respons direct call (tanpa proxy) masih format asli Anthropic { content: [...] }.
  const text = typeof data.text === "string" ? data.text : (data.content || []).map((b) => (b.type === "text" ? b.text : "")).filter(Boolean).join("\n");
  const clean = text.replace(/```json|```/g, "").trim();
  return JSON.parse(clean);
}

// ---------------- AI gratis (Cloudflare Workers AI) — hanya tersedia lewat proxy, hanya untuk narasi teks ----------------
async function askCloudflareAI({ system, userContent }, proxyUrl) {
  if (!proxyUrl) throw new Error('AI gratis butuh Proxy URL diisi dulu di pengaturan');
  const endpoint = proxyUrl.replace(/\/+$/, "") + "/narrate_cf";
  const res = await fetch(endpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function narrateSignal(instrument, style, sig, proxyUrl, mode) {
  ...
  if (mode === "cfai") return askCloudflareAI({ system, userContent }, proxyUrl);
...
function AnalisaSendiri({ proxyUrl, narrationMode }) {
  ...
  const handleGenerate = async () => {
    ...
    if (proxyUrl && liveCandles && GOLDAPI_SYMBOL[instrument]) {
      try {
        const precisePrice = await fetchGoldApiPrice(proxyUrl, GOLDAPI_SYMBOL[instrument]);
        liveCandles = applyPriceOverride(liveCandles, precisePrice);
        source = "goldapi";
      } catch (e) {
        // GoldAPI gagal, tetap lanjut pakai harga TwelveData saja tanpa override.
      }
    }
          ...
          <div className="flex justify-end mb-2">
            <span
              className={
                ...
                (dataSource !== "simulasi" ? "text-[#34C77B] border-[#34C77B]/40" : "text-[#8D8B93] border-[#26262C]")
              }
            >
  ...
  return (
    <div className="mb-6">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-1.5 text-xs text-[#8D8B93] hover:text-[#C9C7CF] mx-auto block">
        ...
        {proxyUrl ? "Data live aktif" : "Pakai data simulasi"} · {narrationMode === "ai" ? "narasi Claude" : narrationMode === "cfai" ? "narasi AI gratis" : "narasi template"} · pengaturan
      </button>
      {open && (
        <div className="mt-3 rounded-lg border border-[#26262C] bg-[#111114] p-4 space-y-4">
          ...
          <div className="pt-3 border-t border-[#26262C]">
            ...
            <div className="grid grid-cols-3 gap-2 mt-1.5">
              ...
              >
                Claude AI
                <div className="text-[9px] mt-0.5 opacity-70">Butuh kredit</div>
              </button>
              <button
                onClick={() => setNarrationMode("cfai")}
                className={
                  "rounded-md border px-2 py-2 text-xs transition-colors " +
                  (narrationMode === "cfai" ? "border-[#C9A45C] text-[#F1EFE9] bg-[#C9A45C]/10" : "border-[#26262C] text-[#8D8B93]")
                }
              >
                AI Gratis
                <div className="text-[9px] mt-0.5 opacity-70">Cloudflare AI</div>
              </button>
              <button
              ...
              >
                Template
                <div className="text-[9px] mt-0.5 opacity-70">Tanpa AI</div>
              </button>
            </div>
          </div>

          <p className="text-[10px] text-[#55545C] leading-relaxed">
            Proxy dipakai untuk harga live TwelveData/GoldAPI, dan untuk narasi AI (Claude atau AI Gratis Cloudflare). Key/token tidak pernah ditaruh di sini — hanya tersimpan di server proxy kamu. Tab "Analisa Chart" selalu pakai Claude AI (satu-satunya yang bisa baca gambar), berapa pun mode yang dipilih di sini.
          </p>
        </div>
      )}
    </div>
